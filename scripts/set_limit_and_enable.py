...
async def main():
    if len(sys.argv) < 2:
        ...
        print("\nEjemplos:")
        print("  python set_limit_and_enable.py 50      # Limitar al 50%")
        print("  python set_limit_and_enable.py 75      # Limitar al 75%")
        print("  python set_limit_and_enable.py 100     # Limitar al 100%")
    ...
    async with ModbusController(config_path) as controller:
        print(f"\n=== LIMITAR POTENCIA AL {valor}% Y HABILITAR ===\n")
        ...
        print(f"  Limitación: {limit_inicial:.1f}%")
        ...
        print(f"      Verificado: {limit_verificado:.1f}%")
        ...
        if int(enable_final) == 1 and abs(limit_final - valor) < 0.1 and int(timeout_final) == 0:
            ...
            if abs(limit_final - valor) >= 0.1:
                print(f"   - Límite debería ser {valor}% pero es {limit_final:.1f}%")
